...
class RotateGripper : public BT::StatefulActionNode
{
public:
  RotateGripper(const std::string & name, const BT::NodeConfig & config)
  : BT::StatefulActionNode(name, config)
  {
    config.blackboard->get("node", node_);
    pub_ = node_->create_publisher<std_msgs::msg::String>("/topico_garra", 10);
    sub_ = node_->create_subscription<std_msgs::msg::String>(
      "/topico_garra_status", 10,
      [this](std_msgs::msg::String::SharedPtr msg) {
        if (msg->data == expected_reply_) { done_ = true; }
      });
  }
